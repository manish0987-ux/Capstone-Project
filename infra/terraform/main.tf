terraform {
  ...
  backend "s3" {
    bucket         = "devops-accelerator-platform-tf-state-manish123"
...
resource "aws_iam_policy" "presign_lambda_policy" {
  name_prefix = "DevOps-Accelerator-Presign-Lambda-Policy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "devops_accelerator_lambda_sns_policy" {
  name_prefix = "devops-accelerator-lambda-sns-publish-policy"
